/*
Simple neuronal network for AND, OR etc.
...
#define AND			1
#define OR			2
// expected result, one from above
#define ANN_TYPE	AND

//! number of samples to train the network
const int				nTrainingSamples = 1000;
...
int main()
{
	cout << "Neuronal network for AND, OR etc" << endl;
	...
	// AND, OR but not XOR also works with topology = {2, 1} and no BIAS neurons !
	vector<size_t>		topology = {2, 4, 1};
	NeuralNetwork		ANN;
	...
	while ( ++trainingPass <= nTrainingSamples )
	{
		...
		inputVals.push_back((nInput & 2) != 0);			// fill input and target vectors with ints, 0 or 1
		inputVals.push_back((nInput & 1) != 0);
#if ANN_TYPE == AND
		targetVals.push_back(nInput == 3);				// expected AND result
#elif ANN_TYPE == OR
		targetVals.push_back(nInput != 0);				// expected OR result
#endif
