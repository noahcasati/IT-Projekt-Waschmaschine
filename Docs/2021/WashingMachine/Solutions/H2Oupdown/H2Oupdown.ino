...
Simple washing machine water level controller.

<pre>
(c)2013-2021 Ingenieurbüro Dr. Friedrich Haase
...
Nov 2020  Dr. Haase  water level up/down example
Sept 2021 Dr. Haase  changed to ESP32
</pre>
*/
...
int             nCount100msec = 0;
                                                ...
                                                // I²C attributes
//! Address of used slave
const int       I2C_PLANT_ADDR = 11;
...
const int       nWaterIntake = 5;               ///< water intake valve
const int       nWaterPump = 2;                 ///< water pump
const int       nHeating = 4;                   ///< heating

                                                // static const PLC IO input numbers
const int       nDoorClosed = 15;               ///< door closed sensor
...
const char     szBanner[] = "# Washing Machine Controller, Water level up/down";

                                                // controller data
int             nState = 0;                     ///< state machine state

void Controller()
{

  if ( ! digitalRead(nDoorClosed) )
  {
    digitalWrite(nWaterIntake, 0);
    digitalWrite(nWaterPump, 0);
    return;                                       // wait until door closed
  }

  switch ( nState )
  {
  case 0:                                         // water up
    digitalWrite(nWaterIntake, 1);
    digitalWrite(nWaterPump, 0);
    if ( dWaterLevel >= 5.0 )
      nState = 1;
    break;
...
void setup()
{
  ...
  I2C_Master_Setup(I2C_FREQUENCY, SDA_PIN, SCL_PIN); // start I²C master
}
...
void Task_100ms()
{
  ...
  int           nSlaveNo;
  int           nResult = I2C_GetResponse(&nSlaveNo, szResponse);
  if ( nResult >= 0 )
  {
